...
- **多 Bot 同群互相不可见**：如果两个 Bot 在同一个群里，默认设置下它们收不到对方的消息。飞书开放平台 → 应用功能 → 机器人 → 群聊接收消息模式，必须设为「**接收群聊中所有消息**」，默认是「仅接收 @ 机器人消息」。两个 Bot 都要改，否则 Bot A 看不到 Bot B @ 它的消息，Bot B 也看不到 Bot A 的回复。
