...
## Quick Throwaway Sketching (Low-Fidelity Variant Exploration)

When the user wants to **see options fast** without a full design process — quick HTML mockups to compare directions:

### Sketch Method
```
intake → 2-3 variants → head-to-head → pick winner
```

1. **Intake** — Ask (if not already given): the vibe, a reference app/site, the core action.
2. **Variants** — Produce 2-3 standalone HTML files, each taking a different design stance (density, emphasis, aesthetic, layout) — not just color swaps.
3. **Head-to-head** — Present a comparison table. Opinionate — say which variant wins and why.
4. **Winner** — Let the user pick, combine, or iterate.

### Variants directory
```
sketches/
├── NNN-calm-editorial/index.html + README.md
├── NNN-utilitarian-dense/index.html + README.md
└── NNN-playful-split/index.html + README.md
```

### Interactivity bar (minimum for a useful sketch)
1. Click a primary action → something visible happens
2. One meaningful state transition (filter, toggle, open/close)
3. Hover recognizable affordances (buttons, rows, tabs)

### Theming (shared tokens)
If the project has an identity, put shared CSS variables in `sketches/themes/tokens.css` (3 colors, 1 font, 1 radius — don't over-tokenize).

### Frontier mode (what to sketch next)
Propose 2-4 candidates from: consistency gaps across sketches, unsketched screens, state coverage (empty/loading/error), responsive gaps, interaction patterns.

Sketch output directory: `sketches/` (or `.planning/sketches/` for GSD conventions).
