...
- `references/china-network-fallback.md` — Use Python + Chinese image API when `image_generate` is unavailable
...
6. **image_generate may not be available** — on Windows Hermes Desktop without Hermes Web UI configured, `image_generate` does not exist. In Chinese network environments, the default `fun-codex` provider may also be unreachable. **Fallback**: use `execute_code` with Python's `urllib`/`requests` to call a Chinese image-generation API directly (SiliconFlow, 通义千问, etc.). See `references/china-network-fallback.md` for the verified integration pattern.
7. **Windows file-writing fallback** — if `write_file` fails due to the MSYS/bash space-in-path issue in the session working directory, use `execute_code` with Python's built-in `open()` and `os.makedirs()` instead of the Hermes `write_file` tool. The Python subprocess is not affected by the bash `cd` failure.
