...
Web 管理面板，在浏览器中管理 Hermes Agent 的所有功能。

```bash
hermes dashboard              # 启动（默认端口 9119）
hermes dashboard --stop       # 停止
hermes dashboard --status     # 查看运行状态
hermes dashboard --host 0.0.0.0  # 允许远程访问（需配合 --insecure，注意安全）
hermes dashboard --tui        # 启用浏览器内嵌 Chat 功能
```
...
| 页面 | 功能 |
|------|------|
| SESSIONS | 浏览/搜索/删除历史会话（显示模型、消息数、时间、来源平台） |
| ANALYTICS | 使用统计、Token 消耗分析 |
| MODELS | 查看/切换当前模型和 Provider |
| LOGS | 实时查看运行日志 |
| CRON | 管理定时任务（增删改查） |
| SKILLS | 浏览/搜索/安装技能 |
| PLUGINS | 管理插件 |
| PROFILES : MULTI AGENTS | 管理多配置文件（实例隔离） |
| CONFIG | Web 编辑 config.yaml |
| KEYS | 管理 API Key 和凭证池 |
| DOCUMENTATION | 内嵌官方文档 |

#### 亮点功能

- **一键重启 Gateway** / **一键更新 Hermes**
- **主题切换**（深色/浅色）+ **中英文界面**
- **插件集成**：Achievements（成就系统）、Kanban（看板协作）

#### Profiles 多 Agent 配置

Profiles 是 Hermes 的**多实例隔离系统**。每个 Profile 独立拥有自己的 config、skills、sessions、memory、cron。
...
hermes --profile NAME            # 启动时指定 profile
```

**Dashboard 操作**：在 PROFILES : MULTI AGENTS 页面可以完成上述所有操作（创建/删除/切换/导出导入）。
...
| **Dashboard** | `hermes dashboard` (port 9119) or `references/hermes-dashboard-guide.md` |
