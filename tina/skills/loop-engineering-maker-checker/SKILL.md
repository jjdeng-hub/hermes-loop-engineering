...
## 变体三：解决方案对抗式审查（Pre-delivery Adversarial Review）

当用户（如 Jerry）要求你在提出解决方案之前用子任务进行对抗性审查时使用此变体。目标不是验证"做得对不对"，而是**主动寻找方案的漏洞、假设缺陷、和反例**。

### 触发条件
- Jerry 明确要求对抗式审查
- 高影响决策（修改配置、删除数据、选型决策）
- 之前类似方案被用户打回过的场景

### Step 1: 拟方案
基于已有知识，形成初步方案。输出到临时文件。

### Step 2: delegate_task 对抗式审查
用 `delegate_task` 起一个 adversarial checker，prompt 如下：

> 你是一个敌对的审查者。你的工作是**破坏**下面的方案，不是美化它。
>
> 请从以下角度攻击这个方案：
> 1. **逻辑关** — 前提成立吗？因果链合理吗？有什么隐藏假设？
> 2. **事实关** — 有哪些数据/断言没有证据支持？可能是你猜的但包装成了事实？
> 3. **简化关** — 有没有更简单的做法？能不能用更少的步骤达到同样效果？
> 4. **遗漏关** — 方案没考虑到什么？最可能翻车的是什么？
>
> 给出具体的攻击点，不要泛泛而谈。

### Step 3: 对账与修正
- 将 adversarial checker 的输出与你的方案对比
- 对每个攻击点：
  - ✅ 有道理 → 修正方案
  - ❌ 不成立 → 准备反驳理由（但在向用户交付时直接跳过，只说结论）
- 修正后的方案才是最终交付版本

### 与标准 Maker/Checker 的区别

| 维度 | Maker/Checker | Adversarial Review |
|------|--------------|-------------------|
| 目标 | 验证正确性和完整性 | 主动寻找漏洞和缺陷 |
| 视角 | 中立客观 | 敌对、找茬 |
| 输出 | pass/fail + 问题清单 | 攻击点 + 反例 |
| 适用 | 每轮交付前 | 高影响决策 + 用户明确要求时 |

### 案例验证（2026-07-03）

Jerry 要求对 provider 配置方案做对抗式审查。delegate_task 查出了 `providers.custom.sn-sensenova` 的三层嵌套错误——这正是后续所有模型配置问题的根因。如果只做常规自检，这个错误会继续隐藏到下一次报错。
