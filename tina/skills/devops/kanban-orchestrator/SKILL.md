...
description: Hermes Kanban system — orchestrator decomposition playbook and worker pitfalls/examples. Covers both the orchestrator role (plan, decompose, route) and the worker role (execute, handoff, retry).
version: 4.0.0
...
# Worker Guide — Execution, Handoff, Pitfalls

> The **lifecycle** (6 steps: orient → work → heartbeat → block/complete) is auto-injected into every worker's system prompt as `KANBAN_GUIDANCE`. This section is the deeper detail.

## Workspace handling

| Kind | What it is | How to work |
|---|---|---|
| `scratch` | Fresh tmp dir, yours alone | Read/write freely; GC'd when the task is archived. |
| `dir:<path>` | Shared persistent directory | Other runs read what you write. Long-lived state. |
| `worktree` | Git worktree | If `.git` doesn't exist, run `git worktree add <path> ${HERMES_KANBAN_BRANCH:-wt/$HERMES_KANBAN_TASK}` from the main repo first. |
...
```
Use `kanban_complete` for terminal tasks; `kanban_block(reason="review-required: ...")` for anything needing human eyes.
...
Pass `created_cards=[...]` on `kanban_complete` for any kanban tasks your run produced. Only list ids captured from successful `kanban_create` return values — never invent ids from prose or paste from earlier runs. The kernel verifies each id; phantom ids block the completion.

## Good block reasons

Bad: `"stuck"` — no context. Good: one sentence naming the specific decision. Context goes in a comment:

```python
kanban_comment(body="Full context: ...")
kanban_block(reason="Rate limit key choice: IP (simple) or user_id (requires auth)?")
```

## Heartbeats

Good: `"epoch 12/50, loss 0.31"`, `"uploaded 47/120 videos"`. Bad: `"still working"`, empty. Every few minutes max; skip for tasks under ~2 minutes.

## Retry diagnostics

If `kanban_show` returns prior runs:
- `timed_out` — previous attempt hit max_runtime. Chunk the work.
- `crashed` — OOM or segfault. Reduce memory.
- `spawn_failed` — config issue (missing credential, bad PATH). Block for human.
...
- **Task state can change between dispatch and startup.** Always `kanban_show` first. If it says `blocked` or `archived`, stop.
- **Workspace may have stale artifacts.** Read the comment thread for context.
- **Use kanban tools, not CLI.** `kanban_*` tools work across all backends (Docker, Modal, SSH). `hermes kanban <verb>` from terminal may fail in containers.
