...
### Isolation rules for comparison experiments

**CRITICAL**: Two modes being compared must be completely independent. Run each in a separate `delegate_task` call. Neither knows the other exists. They share the same input data but get different instructions.

**Self-contained outputs**: Each mode writes its own report with embedded metrics. Do NOT rely on yourself to observe during the run.

### Pitfall: Verifier Theater

If the "checker" is the same model without an external standard, its blind spots converge with the maker's. Give the checker an explicit quantitative checklist. The checker's job: "find reasons to reject," not "confirm."

Known result: in a knowledge base quality audit (10 articles), a single self-verifying agent missed systematic alt-text issues (439/447 articles affected) despite writing a 269-line report. A two-pass approach (free audit + structured checklist) caught them.
